## LiquidityMiningCampaignPayment



Extension of a liquidity mining campaign.
...
address _paymentContract; // Address of the payment contract
```
...
Overrides the old start method so that it can only be called by the payment contract
...
Overrides the old cancel method so that it can only be called by the payment contract
...
Overrides the old extend method so that it can only be called by the payment contract
...
Overrides the cancel extend method so that it can only be called by the payment contract
