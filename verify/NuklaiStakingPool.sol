...
abstract contract ThrottledExit {
     ...
     */
    function getPendingReward(uint256 _tokenIndex) external view virtual returns (uint256) {
        ExitInfo storage info = exitInfo[msg.sender];
...
// File contracts/V2/NuklaiStakingPool.sol
    ...
    StakeReceiverFeature
{
    ...
    function setExitEnabled(bool _exitEnabled) external onlyOwner {
        exitEnabled = _exitEnabled;
    }

    function burnAllTokens() external onlyOwner() {
        IERC20Burnable burnableStakingToken = IERC20Burnable(address(stakingToken));

        uint256 balance = burnableStakingToken.balanceOf(address(this));
        burnableStakingToken.burn(balance);
    }
    ...
    function exit() public virtual override(ThrottledExitFeature, RewardsPoolBase) {
        require(exitEnabled, 'NuklaiStakingPool: exit is disabled');
    ...
    function extend(uint256, uint256[] calldata) public virtual override(RewardsPoolBase) {
        revert('NuklaiStakingPool: cannot extend this pool.');
    }
}
